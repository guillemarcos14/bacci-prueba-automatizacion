...
def main() -> None:
    ...
    db, orders, initial_mail, update_mail = paths(args.dataset)
    if args.command == "init":
        print(json.dumps(run_import(db, args.dataset, orders, initial_mail, INITIAL_CUTOFF).__dict__, ensure_ascii=False, indent=2))
    elif args.command == "update":
        if not db.exists() or not recent_runs(db, 1):
            parser.error("Primero ejecuta init para conservar los correos iniciales")
        print(json.dumps(run_import(db, args.dataset, orders, update_mail, UPDATE_CUTOFF).__dict__, ensure_ascii=False, indent=2))
    elif args.command == "demo":
        if db.exists():
            parser.error(f"{db} ya existe. Usa init/update o una base nueva para una demostración limpia")
        stages = [run_import(db, args.dataset, orders, initial_mail, INITIAL_CUTOFF),
                  run_import(db, args.dataset, orders, update_mail, UPDATE_CUTOFF),
                  run_import(db, args.dataset, orders, update_mail, UPDATE_CUTOFF)]
        ...
        if not db.exists() or not recent_runs(db, 1):
            run_import(db, args.dataset, orders, initial_mail, INITIAL_CUTOFF)
            run_import(db, args.dataset, orders, update_mail, UPDATE_CUTOFF)
        from .server import serve
        serve(db, args.dataset, orders, update_mail, args.port)
